        ...
        while not self.stop_flag:
            ...
            if current_time - self.last_detection_time >= FRAME_PROCESSING['DETECTION_INTERVAL']:
                ...
                if priority_target:
                    ...
                    # Добавляем надпись "ЦЕЛЬ" над прямоугольником
                    frame = put_russian_text(frame, 
                                          "ЦЕЛЬ",
                                          (x1, y1 - TARGET['LABEL_OFFSET']),
                                          font_scale=TARGET['LABEL_FONT_SCALE'],
                                          color=TARGET['LABEL_COLOR'])
                    
                    # Рисуем белый прямоугольник для фона
                    cv2.rectangle(frame, 
                                TARGET['POSITION'],
                    ...
                    frame = put_russian_text(
                        ...
                        (TARGET['POSITION'][0] + TARGET['PADDING'],
                         TARGET['POSITION'][1] + TARGET['TEXT_OFFSET_Y1'] - 10),  # Немного поднимаем текст
                    ...
                    frame = put_russian_text(
                        ...
                        coord_text,
                        (TARGET['POSITION'][0] + TARGET['PADDING'],
                         TARGET['POSITION'][1] + TARGET['TEXT_OFFSET_Y2'] - 10),  # Немного поднимаем текст
                        TARGET['INFO_FONT_SCALE'],
                        TARGET['TEXT_COLOR']
                    )
                
                if priority_target:
                    # Получаем координаты цели относительно центра кадра
                    target_center_x = deviation[0] - frame.shape[1] / 2
                    target_center_y = deviation[1] - frame.shape[0] / 2
                    
                    # Если цель не в центре (с учетом допуска)
                    if not self.turel.is_on_target(target_center_x, target_center_y):
                        # Двигаем турель к цели
                        self.turel.move_to_target(target_center_x, target_center_y)
